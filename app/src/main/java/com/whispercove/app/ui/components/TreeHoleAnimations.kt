 ...
 * 树洞信件漂流站 动画组件
 * 实现三种核心动效：
 * 1. 拆信件动效：帧动画（2帧），300ms
 * 2. 打包信件动效：帧动画（1帧），200ms
 * 3. 标签选中动效：颜色切换，100ms
 */
object TreeHoleAnimations {
...
object TreeHoleIcons {
    ...
    fun DrawScope.drawSealIcon(size: Float, fillColor: Color, borderColor: Color) {
        ...
        // 绘制简单的T字母（TreeHole首字母）
        val tPath = Path().apply {
            val tWidth = radius * 1.2f
            val tHeight = radius * 1.2f
            val startX = center.x - tWidth / 2
            val startY = center.y - tHeight / 2
            
            // 绘制T
            moveTo(startX, startY)
            lineTo(startX + tWidth, startY)
            moveTo(center.x, startY)
            lineTo(center.x, startY + tHeight)
        }
